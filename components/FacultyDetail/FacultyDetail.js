...
const FacultyDetail = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        departments.forEach(dept => {
          deptMap[dept] = data.filter(item => item.dept === dept);
        });
  ...
  const renderDepartmentSection = (department) => {
    return (
      <div className="flex flex-col justify-between space-y-10">
        <h1 className="text-[#201F54] font-semibold text-2xl pl-[7rem]">
        ...
        <div className="grid grid-cols-1 px-4 md:grid-cols-4 justify-between items-center mx-auto gap-20">
          {deptData[department]?.map((faculty) => (
            <FacultyCard key={faculty.id} {...faculty} isBig={false} />
          ))}
        </div>
      </div>
    );
