...
const AddLinks = () => {
  ...
  return (
    ...
    <tbody className="flex pt-[5rem] flex-col border-[2px]">
        ...
        {linksData?.map(i => (
            <tr key={i.id} className="flex justify-between items-center justify-center py-[1rem]">
                <td className="flex-1">{i.id}</td>    
                <td className="flex-1">{i.link_title}</td>    
                <td className="flex-1">{i.link}</td>    
                <td onClick={() => linkDeleteHandler(i)} className="flex-1 rounded-3xl text-red-500 cursor-pointer ">{'Delete'}</td>    
            </tr>
        ))}
    </tbody>
</table>
