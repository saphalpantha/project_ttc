    ...
    <div>
          ...
          {props.list.map((i, indx) => {
            return (
              <Fragment>
                <tr
                  key={i.id}
                  className="flex justify-between  mx-[3.8rem] gap-[10rem] py-4"
                >
      ...
      <div className="w-[100%] flex flex-col">
        <button
          ...
          disabled={!change}
          type="submit"
          className={`border-2 mx-auto cursor-pointer  border-[#201F54] opacity-100 px-6 py-2 w-fit ${
            change
              ? "bg-[#201F54] text-white "
              : " cursor-not-allowed opacity-[0.5]"
          }`}
        >
          {!loading ? "Finish Changes" : "Finishing Changes"}
        </button>
      </div>
    </div>
  );
