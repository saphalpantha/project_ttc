...
const Slider = () => {
  ...
  const pushToDb  = async (e) => {
    ...
    try {
      setLoading(true)
      await axios.post(`/api/slider/upload`, formData, {
        headers: {
...
<button className='px-4 absolute mt-[1rem] top-[0rem]  right-[3.5rem] hover:bg-[#201F54] hover:text-white  w-fit py-2 border-[1px] rounded-full border-[#201F54] transition-all duration-300 ease-in-out' onClick={() => setOpenPostForm(prev => !prev)}>{openPostForm ? 'Close' : 'Add New'}</button>
</div>
