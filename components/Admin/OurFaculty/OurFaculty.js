...
//         <button
//           className="px-4 absolute  top-[0.8rem] right-[3.5rem] hover:bg-[#201F54] hover:text-white  w-fit py-2 border-[1px] rounded-full border-[#201F54] transition-all duration-300 ease-in-out"
  ...
  return (
    <div className="pl-[22%] w-[100vw] py-10">
      <div className="py-5 w-full justify-end relative flex flex-col">
        <button
          className="px-4 absolute top-[0.8rem] right-[3.5rem] hover:bg-[#201F54] hover:text-white w-fit py-2 border-[1px] rounded-full border-[#201F54] transition-all duration-300 ease-in-out"
          onClick={() => setFieldValue("openPostForm", !values.openPostForm)}
        >
          {values.openPostForm ? "Close" : "Add New"}
        </button>
      </div>
      {values.openPostForm ? (
        <form
