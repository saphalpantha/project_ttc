...
const OthersCard = ({img,title, link}) => {
  return(
    ...
    <img className="w-[2.3rem]  h-[2.3rem]" src={`/images/${img}.svg`}></img>
...
const others = [
  ...
  {id:'i1', title:'Add Spotlight', img:'spotlight', link:'/admin/spotlight'},
  {id:'i1', title:'Add News & Notice', img:'megaphone', link:'/admin/newsnotice'},
  {id:'i1', title:'Gallery Settings', img:'gallery', link:'/admin/gallary'},
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    fetch("/api/admission-forms")
      ...
      .then((data) => {
        ...
        setTotalManagement(allForms.filter( i => i.faculty == 'Management').length)
  ...
  const events = [
    {id:'i1', title:'Total Admission Form', count:totalAdmissions, img:'total-admission'},
    {id:'i2', title:'Science Form', count:totalScience, img:'total-science'},
    {id:'i3', title:'Management Form', count:totalManagement, img:'total-management'},
  ...
  return (
    <div className={`flex   flex-col min-h-screen gap-10 ${classes.main}`}>
      <div className="flex gap-4  items-center ">
        <img className="w-[2rem] h-[2rem]" src={"/images/dashboard-icon.svg"}></img>
      ...
      <div className="flex flex-col gap-2">  
          ...
          <div className="flex flex-shrink-1 flex-wrap  gap-4">
            {events.map(i => <EventCard img={i.img} count={i.count} title={i.title}/>)}
          </div>
      </div>
      <div className="flex flex-col gap-2">  
          ...
          <div className="flex flex-shrink-1 flex-wrap  gap-4">
            {others.map(i => <OthersCard img={i.img} title={i.title} link={i.link}/>)}
          </div>
      </div>
    </div>
  );
