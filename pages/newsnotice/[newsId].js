    ...
    <Container>
      <div className="flex flex-col justify-center mx-2 mx:px-32 gap-5 items-center py-[2rem] pb-[5rem]">
        <div className=" h-[50vh] md:h-[100vh]">
          <img className="w-[100%] h-[100%]" src={props.singleNews.img}></img>
        </div>
...
export const getStaticProps = async (context) => {
  ...
  const getSingleNotice = newsData.find(item => item.id === id);
  return{
    props:{
...
export const getStaticPaths = async () => {
  ...
  const paths = allItems.map(user => ({params:{newsId:user.id}}));
  return{
    paths:paths,
    fallback:false,
  }
